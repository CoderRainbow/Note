 ...
 *   配合下面的代码看上面的注释。
 */
public class T12_Singleton03 {

    private static volatile T12_Singleton03 INSTANCE;

    private T12_Singleton03(){};

    // 双重检查单例：防止里面会有业务代码，如果有业务代码且锁加在方法上是不合适的。所以改成下面的样子。
    private T12_Singleton03 getINSTANCE() {
        // 可以在这里写业务代码
        if(INSTANCE == null){
            synchronized (T12_Singleton03.class) {
                if(INSTANCE == null) {
                    try {
                        Thread.sleep(1);
    ...
    public static void main(String[] args) {
        T12_Singleton03 s1 = new T12_Singleton03().getINSTANCE();
        T12_Singleton03 s2 = new T12_Singleton03().getINSTANCE();
